...
def binary_cross_entropy_loss(out, y):
    ...
    log_softmax_out = minitorch.nn.logsoftmax(out, 0)
    log_softmax_ones = minitorch.nn.logsoftmax(ones - out, 0)
    loss = -(y * log_softmax_out + (ones - y) * log_softmax_ones).mean()
    return loss
...
class Linear(minitorch.Module):
    def __init__(self, in_size, out_size):
        ...
        self.weights = RParam(in_size, out_size)  # (in_size, out_size)
        self.bias = RParam(out_size)           # (out_size,)
    ...
    def forward(self, x):
        ...
        output = x @ weights + self.bias.value
...
class Network(minitorch.Module):
    ...
    def forward(self, embeddings):
        ...
        embeddings = minitorch.nn.dropout(embeddings, self.dropout_prob)
        embeddings = self.linear2(embeddings)
        embeddings = embeddings.sigmoid()
        return embeddings.view(batch)
...
class SentenceSentimentTrain:
    ...
    def train(
        ...
        train_accuracy = []
        validation_accuracy = []
        for epoch in tqdm(range(1, max_epochs + 1), desc="Training Epoch"):
            total_loss = 0.0
            n_batches = 0
            ...
            if data_val is not None:
                ...
                for val_start in range(0, n_val, val_batch_size):
                    val_end = min(val_start + val_batch_size, n_val)
                    x = minitorch.tensor(X_val[val_start:val_end], backend=BACKEND)
                    y = minitorch.tensor(y_val[val_start:val_end], backend=BACKEND)
                    out = model(x)
